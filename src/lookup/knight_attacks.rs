...
pub fn generate_knight_attacks() -> [Bitboard; 64] {
    ...
    for square_index in 0..NUM_SQUARES {
        knight_attacks[square_index as usize] = get_attack_bb(Square::new(square_index));
    }
    knight_attacks
}
...
fn get_attack_bb(square: Square) -> Bitboard {
...
mod tests {
    ...
    use crate::lookup::knight_attacks::get_attack_bb;

    #[test]
    fn get_attack_bb_returns_bitboard_with_attacked_bits_set() {
        for square_index in 0..NUM_SQUARES {
            // get attacks for square
            let attack_bb = get_attack_bb(Square::new(square_index));
        ...
        assert_eq!(0x284400442800,get_attack_bb(E4).value);
        assert_eq!(0x20400,get_attack_bb(A1).value);
        assert_eq!(0x402000,get_attack_bb(H1).value);
        assert_eq!(0x4020000000000,get_attack_bb(A8).value);
        assert_eq!(0x20400000000000,get_attack_bb(H8).value);
        assert_eq!(0x22140000000000,get_attack_bb(D8).value);
        assert_eq!(0xa01000,get_attack_bb(G1).value);
        assert_eq!(0x508000805,get_attack_bb(B3).value);
        assert_eq!(0x5088008850000000,get_attack_bb(F6).value);
        assert_eq!(0x1100110a00000000,get_attack_bb(C7).value);
    }
}