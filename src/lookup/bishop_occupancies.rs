...
impl LookupTable {
    ...
    pub(super) fn generate_bishop_occupancies() -> [Bitboard; 64] {
        let mut bishop_occupancies = [Bitboard::new(0); 64];
        for square_index in 0..NUM_SQUARES {
            bishop_occupancies[square_index as usize] = Self::get_bishop_occupancy_bb(Square::new(square_index));
        }
        bishop_occupancies
    }

    /// Returns the occupancy bitboard for a bishop on a specified square.
    fn get_bishop_occupancy_bb(square: Square) -> Bitboard {
...
mod tests {
    ...
    #[test]
    fn get_bishop_occupancy_bb_returns_bitboard_with_relevant_occupancy_bits_set() {
        for square_index in 0..NUM_SQUARES {
            // get occupancies for square
            let occupancy_bb = LookupTable::get_bishop_occupancy_bb(Square::new(square_index));
        ...
        assert_eq!(0x2442800284400, LookupTable::get_bishop_occupancy_bb(E4).value);
        assert_eq!(0x40201008040200, LookupTable::get_bishop_occupancy_bb(A1).value);
        assert_eq!(0x2040810204000, LookupTable::get_bishop_occupancy_bb(H1).value);
        assert_eq!(0x2040810204000, LookupTable::get_bishop_occupancy_bb(A8).value);
        assert_eq!(0x40201008040200, LookupTable::get_bishop_occupancy_bb(H8).value);
        assert_eq!(0x14224000000000, LookupTable::get_bishop_occupancy_bb(D8).value);
        assert_eq!(0x20408102000, LookupTable::get_bishop_occupancy_bb(G1).value);
        assert_eq!(0x20100804000400, LookupTable::get_bishop_occupancy_bb(B3).value);
        assert_eq!(0x50005008040200, LookupTable::get_bishop_occupancy_bb(F6).value);
        assert_eq!(0xa1020400000, LookupTable::get_bishop_occupancy_bb(C7).value);
    }

    #[test]
    fn get_bishop_occupancy_bb_returns_bitboard_with_a_h_files_and_first_eighth_rank_and_bishop_square_unset() {
        for square_index in 0..NUM_SQUARES {
            // get occupancies for square
            let occupancy_bb = LookupTable::get_bishop_occupancy_bb(Square::new(square_index));
