...
pub fn generate_rook_occupancies() -> [Bitboard; 64] {
    let mut rook_occupancies = [Bitboard::new(0); 64];
    for square_index in 0..NUM_SQUARES {
        rook_occupancies[square_index as usize] = get_occupancy_bb(Square::new(square_index));
    }
    rook_occupancies
}
...
mod tests {
    ...
    use crate::lookup::rook_occupancies::get_occupancy_bb;
    ...
    fn get_occupancy_bb_returns_bitboard_with_relevant_occupancy_bits_set() {
        ...
        assert_eq!(0x1010106e101000, get_occupancy_bb(E4).value);
        assert_eq!(0x0, get_occupancy_bb(A1).value);
        assert_eq!(0x0, get_occupancy_bb(H1).value);
        assert_eq!(0x0, get_occupancy_bb(A8).value);
        assert_eq!(0x0, get_occupancy_bb(H8).value);
        assert_eq!(0x8080808080800, get_occupancy_bb(D8).value);
        assert_eq!(0x40404040404000, get_occupancy_bb(G1).value);
        assert_eq!(0x20202027c0200, get_occupancy_bb(B3).value);
        assert_eq!(0x205e2020202000, get_occupancy_bb(F6).value);
        assert_eq!(0x7a040404040400, get_occupancy_bb(C7).value);
    }

    #[test]
    fn get_occupancy_bb_returns_bitboard_with_a_h_files_and_first_eighth_rank_and_rook_square_unset() {
        for square_index in 0..NUM_SQUARES {
            // get occupancies for square
            let occupancy_bb = get_occupancy_bb(Square::new(square_index));
