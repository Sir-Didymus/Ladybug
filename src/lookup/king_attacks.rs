...
pub fn generate_king_attacks() -> [Bitboard; 64] {
    ...
    for square_index in 0..NUM_SQUARES {
        king_attacks[square_index as usize] = get_attack_bb(Square::new(square_index));
    }
    king_attacks
}
...
mod tests {
    ...
    use crate::lookup::king_attacks::get_attack_bb;
    ...
    fn get_attack_bb_returns_bitboard_with_attacked_bits_set() {
        ...
        assert_eq!(0x3828380000, get_attack_bb(E4).value);
        assert_eq!(0x302, get_attack_bb(A1).value);
        assert_eq!(0xc040, get_attack_bb(H1).value);
        assert_eq!(0x203000000000000, get_attack_bb(A8).value);
        assert_eq!(0x40c0000000000000, get_attack_bb(H8).value);
        assert_eq!(0x141c000000000000, get_attack_bb(D8).value);
        assert_eq!(0xe0a0, get_attack_bb(G1).value);
        assert_eq!(0x7050700, get_attack_bb(B3).value);
        assert_eq!(0x70507000000000, get_attack_bb(F6).value);
        assert_eq!(0xe0a0e0000000000, get_attack_bb(C7).value);
    }
}